...
type SetUserBrandGoalV2Request struct {
	UserID        int32 `json:"user_id"`
	BrandId       int32 `json:"brand_id"`
	Value         int64 `json:"value"`
	Month         int32 `json:"month"`
...
func (server *Server) SetUserBrandGoalV2(ctx *gin.Context) {
	var request SetUserBrandGoalV2Request
	if err := ctx.ShouldBindJSON(&request); err != nil {
		ctx.JSON(http.StatusBadRequest, errorResponse(err))
		return
	}
	...
	err = server.setUserBrandGoalV2Request(request.UserID, request.BrandId, request.LeadMeasureID, request.Value, period)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, errorResponse(err))
		return
	}
