...
const addGoodToSale = `-- name: AddGoodToSale :exec
...
type AddGoodToSaleParams struct {
	SaleID int32 `json:"sale_id"`
	GoodID int32 `json:"good_id"`
}

func (q *Queries) AddGoodToSale(ctx context.Context, arg AddGoodToSaleParams) error {
	_, err := q.db.ExecContext(ctx, addGoodToSale, arg.SaleID, arg.GoodID)
	return err
}
...
SET user_id      = $1,
    ...
    sale_type_id = $4,
    description  = $5
WHERE id = $6 RETURNING id, user_id, sale_date, amount, sale_type_id, description, created_at
`
...
func (q *Queries) GetSaleSumByBranchByTypeByBrand(ctx context.Context, arg GetSaleSumByBranchByTypeByBrandParams) (GetSaleSumByBranchByTypeByBrandRow, error) {
	row := q.db.QueryRowContext(ctx, getSaleSumByBranchByTypeByBrand,
		arg.SaleDate,
		arg.SaleDate_2,
		arg.ID,
		arg.ID_2,
